...
#args[1] <- "~/Dropbox/NEOF_21-22/crickets/main_flowcell/MEGASAT_output/Output_"
#args[2] <- "~/Dropbox/NEOF_21-22/crickets/main_flowcell/MEGASAT_output/Output_"
...
for(j in 1:length(lociName)){
  ##############################
  possible_genos <- c()
  for (file in list.files(paste0(genotypeDir, "/"))){
    temp <- read.table(paste0(genotypeDir, "/", file), header=T, se=",")
    temp2 <- temp[ temp$Microsatellite == lociName[j], ][2:(ncol(temp)-2)]
    possible_genos <- c(possible_genos, colnames(temp2[,colSums(temp2) > 0]))
  }
  possible_genos <- unique(possible_genos)
  possible_genos <- sort(gsub('X', '', possible_genos))
  #############################
  ...
  for(i in InfileList){
    ...
    ##### add the zero values to the completedb
    for (new_value in possible_genos[which(!possible_genos %in% completedb$variable)]) {
      completedb <- rbind(completedb, c(lociName[[j]], new_value, "0"))
    }
    ##########################
    #split the score
    ...
    if (length(v) == 1) {
      ...
      sum <- as.numeric(max(v,na.rm=TRUE)) +
        as.numeric(sort(v,partial=length(v)-1)[length(v)-1])
    }
    ...
    SISLData$value <- as.numeric(SISLData$value)
    SISLData$variable <- as.numeric(SISLData$variable)
    SISLData <- SISLData[order(SISLData$variable),]
